...
$startIndex = $index.IndexOf('<div id="auth-gate-container"')
$endString = '<!-- MAIN DASHBOARD CONTAINER'
$endIndex = $index.IndexOf($endString)

$before = $index.Substring(0, $startIndex)
$after = $index.Substring($endIndex)

$finalHtml = $before + $wrappedNewContent + "`n`n  " + $after

Set-Content "d:\HAJA LUZ\Antigravity\Dados_IA\scratch\oraculum-saas\public\index.html" $finalHtml -Encoding UTF8
